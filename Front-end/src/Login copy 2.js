...
function Login() {
        ...
        if (signupPassword !== signupConfirmPassword) {
            ...
            try {
                const response = await axios.post('/api/student/register', { 
                    ...
                    studentPassword: signupPassword 
                });
    ...
    const handleLogin = async (e) => {
        ...
        if (loginPassword.length > 0) {
            try {
                ...
                if (response.data.code === 1) {
                    // Store token in local storage and navigate
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            const response = await axios.post('/api/student/forgotpassword/reset', { 
                studentEmail: email, 
                code: verificationCode, 
                newPassword 
            });
    ...
    const handleSendVerificationCode = async (e) => {
        ...
        try {
            const response = await axios.post('/api/student/send-code', { email });
            if (response.data.code === 1) {
